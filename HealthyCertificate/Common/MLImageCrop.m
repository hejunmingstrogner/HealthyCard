...
//  截取区域的宽、高
@property (nonatomic, assign) CGFloat widthofimage;
...
- (instancetype)initWithRatioOfWidthAndHeight:(CGFloat)ratio
{
    if (self = [super init]) {
        self.ratioWithWidthAndHeight = ratio;
        _widthofimage = self.view.bounds.size.width * 2/3;
        _heightofImage = _widthofimage / ratio;
        if (_heightofImage >= self.view.bounds.size.height - 100) {
            _heightofImage = self.view.bounds.size.height - 120;
            _widthofimage = _heightofImage * ratio;
        }
    }
    return self;
}
